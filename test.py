...
model_path = "./exps/r50_deformable_detr/checkpoint0084.pth"  # 模型权重路径
...
def detect_objects(model, image_tensor, threshold=0.5):
    ...
    scores = torch.squeeze(predictions['pred_logits']).sigmoid()    # logits是MLP输出，需要sigmoid函数转换为概率
...
def draw_and_save_results(image, boxes, scores, wh2, ratio_wh, save_path=None):
...
def main():
    ...
    boxes, scores = detect_objects(model, image_tensor, threshold=conf_threshold)
    draw_and_save_results(test_image, boxes, scores, wh2, ratio_wh, save_path)
